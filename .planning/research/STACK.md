...
**Domain:** Embedded Linux network appliance (WiFi bridge, DNS filtering, compression proxy, traffic shaping)
**Researched:** 2026-03-22
**Confidence:** MEDIUM-HIGH (core components well-established; proxy engine requires prototyping)
...
# Firewall / routing
sudo apt install -y nftables ipset

# Captive portal web server
# Download Caddy ARM64 binary from https://caddyserver.com/download
wget "https://caddyserver.com/api/download?os=linux&arch=arm64" -O /usr/local/bin/caddy
...
wget https://unpkg.com/htmx.org@2.0.0/dist/htmx.min.js -O /var/www/skygate/htmx.min.js
```
...
```bash
# Clone repo
git clone https://github.com/quartermint/skygate.git
cd skygate/server
...
# Pi daemon
go get github.com/r3labs/sse/v2@latest
...
**If running DNS-only mode (Phase B.1, no remote server):**
...
**If running full proxy mode (Phase B.2, with remote server):**
...
**If building pre-configured SD card image (Phase C):**
...
**If running cake-autorate for dynamic QoS:**
...
---
*Stack research for: Aviation bandwidth management appliance (Raspberry Pi + remote proxy)*
*Researched: 2026-03-22*
